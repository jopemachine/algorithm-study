// 백준 알고리즘 강의 중급

#include <iostream>
#include <vector>
#include <string>

using namespace std;

int dx[] = { 0, 0, 1, -1 };
int dy[] = { 1, -1, 0, 0 };

const int LIMIT = 10;

// 만들 수 있는 방법의 수들을 만들어 리턴.
vector<int> gen(int k) {
  vector<int> res(LIMIT);
  // k의 오른쪽 숫자들 부터 두 개씩 읽어나간다.
  for (int i = 0; i < LIMIT; ++i) {
    res[i] = k % 4;
    k >>= 2;
  }
  return res;
}

pair<bool, bool> simulate(vector<string>& map, int k, int& x, int& y) {
  if (map[x][y] == '.') return { false, false };

  bool moved = false;
  while (true) {
    int nx = x + dx[k];
    int ny = y + dy[k];

    if (map[nx][ny] == '#') {
      return { moved, false };
    }
    else if (map[nx][ny] == 'R' || map[nx][ny] == 'B') {
      return { moved, false };
    }
    else if (map[nx][ny] == '.') {
      swap(map[nx][ny], map[x][y]);
      x = nx;
      y = ny;
      moved = true;
    }
    else if (map[nx][ny] == 'O') {
      map[x][y] = '.';
      moved = true;
      return { moved, true };
    }
  }

  return { false, false };
}

int check(vector<string> map, vector<int>& dir) {
  int n = map.size();
  int m = map[0].size();
  int hx, hy, rx, ry, bx, by;

  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < m; ++j) {
      if (map[i][j] == 'O') {
        hx = i;
        hy = j;
      }
      else if (map[i][j] == 'R') {
        rx = i;
        ry = j;
      }
      else if (map[i][j] == 'B') {
        bx = i;
        by = j;
      }
    }
  }

  int cnt = 0;
  for (int k : dir) {
    cnt += 1;
    bool hole1 = false, hole2 = false;
    while (true) {
      auto p1 = simulate(map, k, rx, ry);
      auto p2 = simulate(map, k, bx, by);
      // 둘 다 안 움직임
      if (p1.first == false && p2.first == false) {
        break;
      }
      if (p1.second) hole1 = true;
      if (p2.second) hole2 = true;
    }
    if (hole2) return -1;
    if (hole1) return cnt;
  }

  return -1;
}

bool valid(vector<int>& dir) {
  int l = dir.size();
  for (int i = 0; i + 1 < l; ++i) {
    if (dir[i] == 0 && dir[i + 1] == 1) return false;
    if (dir[i] == 1 && dir[i + 1] == 0) return false;
    if (dir[i] == 2 && dir[i + 1] == 3) return false;
    if (dir[i] == 3 && dir[i + 1] == 2) return false;
    if (dir[i] == dir[i + 1]) return false;
  }

  return true;
}
int main() {
  int n, m;
  cin >> n >> m;
  vector<string> map(n);
  for (string& input: map) {
    cin >> input;
  }

  int ans = -1;

  // 4진법 (두 자리 수) 으로 각각의 움직임들을 나타낼 것이기 때문에, LIMIT * 2번까지 gen을 호출하면 
  // 모든 경우의 수가 만들어진다.
  for (int k = 0; k < (1 << (LIMIT * 2)); ++k) {
    vector<int> dir = gen(k);
    if (!valid(dir)) continue;
    int cur = check(map, dir);
    if (cur == -1) continue;
    if (ans == -1 || ans > cur) ans = cur;
  }

  cout << ans << "\n";
  return 0;
}