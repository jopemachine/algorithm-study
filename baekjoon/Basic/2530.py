...
@ Author: jopemachine
@ Created Date: 12/17/2022, 9:51:05 PM
...
계산하는 프로그램을 작성하시오.
@ Input: 첫째 줄에는 현재 시각이 나온다. 현재 시각은 시 A (0 ≤ A ≤ 23), 분 B (0 ≤ B ≤ 59)와 초 C (0 ≤ C ≤
59)가 정수로 빈칸을 사이에 두고 순서대로 주어진다. 두 번째 줄에는 요리하는 데 필요한 시간 D (0 ≤ D ≤ 500,000)가
초 단위로 주어진다.
@ Output: 첫째 줄에 종료되는 시각의 시, 분, 초을 공백을 사이에 두고 출력한다. (단, 시는 0부터 23까지의 정수이며, 분, 초는 0부터
...
s = (C + ds) % 60
nm = 1 if (C + ds >= 60) else 0

m = (B + dm + nm) % 60
nh = 1 if (B + dm + nm >= 60) else 0
